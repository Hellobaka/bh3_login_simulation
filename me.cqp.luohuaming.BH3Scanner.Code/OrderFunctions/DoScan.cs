...
namespace me.cqp.luohuaming.BH3Scanner.Code.OrderFunctions
{
    public class DoScan : IOrderModel
    {
        ...
        public FunctionResult Progress(CQGroupMessageEventArgs e)//群聊处理
        {
            ...
            var r = ScanResult(e.FromQQ, e.Message.Text);
        ...
        static Dictionary<long, Scan> WaitLogin = new Dictionary<long, Scan>();
        public string ScanResult(long qq, string msg)
        {
            if (WaitLogin.ContainsKey(qq))
            {
                ...
                if (c.Any(x => x.IsImageCQCode))
                {
                    ...
                    using(var http = new Tool.Http.HttpWebClient())
                    {
                        ...
                        string retMsg = "";
                        if (WaitLogin[qq].QRCodeScan(filename))
                        {
                            retMsg = "扫码完成";
                        }
                        else
